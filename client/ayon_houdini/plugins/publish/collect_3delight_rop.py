...
class Collect3DelightROPRenderProducts(plugin.HoudiniInstancePlugin):
    """Collect 3Delight ROP Render Products
    ...
    label = "3Delight ROP Render Products"
    ...
    def process(self, instance):
        ...
        for index in range(1, num_aovs + 1):
            aov_enabled = rop.evalParm("active_layer_{}".format(index))

            # Skip disabled AOVs or AOVs with no separate aov file path
            if not all((aov_enabled)):
                continue
            label = rop.evalParm("aov_name_{}".format(index)).lower()
